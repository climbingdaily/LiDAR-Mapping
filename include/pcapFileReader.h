...
// 数据包长1248字节，通过端口2368上的UDP包发送。
// 数据包由42个字节的协议头、12个数据块(每个100个字节)、一个4字节的时间戳和2个工厂字节组成。
...
{
	unsigned short distance; //2个字节的距离
	unsigned char intensity; //1个字节的反射率
} HDLLaserReturn;

struct HDLFiringData
{
	unsigned short blockIdentifier; //2个字节的字头
	unsigned short rotationalPosition; //2个字节的azimuth
	HDLLaserReturn laserReturns[HDL_LASER_PER_FIRING]; //32份
};

struct HDLDataPacket
{
	HDLFiringData firingData[HDL_FIRING_PER_PKT];
	unsigned int gpsTimestamp; //四个字节
	unsigned char blank1; //一个字节
	unsigned char blank2; //一个字节
};

struct HDLPositionPacket
{
    double  rcv_time;
    char    status;
    double  lat;
...
class veloGPS
{
  public:
	...
	int numSignal;
	Coordinates positions[10]; //每帧最多存十个

	bool addPosition(double lati, double longti)
	{
		if (numSignal < 10)
		{
			positions[numSignal].latitude = lati;
			positions[numSignal].longtitude = longti;
			++numSignal;
		}
		else
		{
...
class pcapReader
{
  ...
  public:
	...
	bool capture(veloFrame &frame, long long &idFrame, double &timeRecieved, std::string gpsFile = "", bool save = true)
	{
		...
		Skip = Skips[idFrame];
		std::string gpsdata;
		std::fstream gpsWrite;
		std::vector<std::string> tokens;

		if (gpsFile != "" && save)
		{
			gpsWrite.open(gpsFile.c_str(), std::ios::out | std::ios::app);
		}
		bool isSetTime = false;
		while (pReader->NextPacket(data, dataLength, timeSinceStart))
		{
			if (dataLength == 512 && gpsFile != "" && save)
			{
				int i = ProcessPositionPacket(const_cast<unsigned char *>(data), dataLength, gpsdata);
				if (i == 0)
				{
					ssplit(gpsdata, tokens, ',');
					if(tokens.size() > 4)
					{
						if (lati.compare(tokens[2]) != 0 || longti.compare(tokens[4]) != 0)
						{
							gpsWrite << idFrame << ' ' << std::fixed << std::setprecision(9) << timeSinceStart << std::endl;
							gpsWrite << gpsdata << std::endl;
						}
						lati = tokens[2];
						longti = tokens[4];
					}
  ...
  private:
	int ReadFrameInformation(void)
	{
		...
		{
			// if (dataLength == 512)
			// 	ProcessPositionPacket(const_cast<unsigned char *>(data), dataLength);
			if (dataLength != 1206) //根据长度判断这是数据段,若是512，则是位置段
			{
				continue;
			}
	...
	void ssplit(const std::string& s, std::vector<std::string>& tokens, const char& delim = ' ') 
	{
		tokens.clear();
	...
	// 读取位置包的代码
	int ProcessPositionPacket(unsigned char *data, std::size_t bytesReceived, std::string &gpsdata)
	{	
		if (bytesReceived != 512)
			return -1;
		char *line = new char[512];
		line = reinterpret_cast<char*>(data);
		char message[77];

		// for (int i=0; i < 512 ; ++i)
		// 	line[i] = data[i]
		// strncpy(line, data, 512);
		HDLPositionPacket rmc;
		if ('$' != line[206])
		{
		...
		{
			for (size_t i = 206; i<283; ++i)
			{
				message[i-206] = line[i];
				// printf("%c", *(line+i));
			}
			gpsdata = message;
			/**
	...
	{
		if (bytesReceived != 1206) //1206+42=1248
		{
			return;
		}
